<form class="space-y-10" wire:submit="submit">
    ...
    <div>
        ...
        <input x-data x-on:select="$wire.setDate($event.detail)"
            x-picker="{
                date: '{{ $form->date }}',
                availability: {{ $this->availabilityJson }}
            }"
            class="mt-6 text-sm bg-slate-100 border-0 rounded-lg px-6 py-4 w-full" placeholder="Choose a date">
    </div>
            ...
            <div class="grid grid-cols-3 md:grid-cols-5 gap-8">
                @if ($this->times->isNotEmpty())
                    @foreach ($this->times as $time)
                        <button wire:click="setTime('{{ $time }}')" type="button" @class([
                            'py-3 px-4 text-sm border border-slate-200 rounded-lg text-center hover:bg-gray-50/75 cursor-pointer',
                            'bg-slate-100 hover:slate-100' => $time === $form->time,
                        ])>
                            {{ $time }}
                        </button>
                    @endforeach
